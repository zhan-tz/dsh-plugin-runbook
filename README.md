...
- **PIPELINE.md 主链骨架**：科研项目里人写好的 `PIPELINE.md`（mermaid 主链 DAG + 状态表）被解析为**视图主角**——阶段卡片按声明顺序成链（含 ✅/🔶/💀/⚠️ 状态角标），磁盘上的真实文件自动挂靠到对应环下方，**声明了但磁盘上没有的文件渲染成虚线幽灵节点（缺口一目了然）**。会话考古永远还原不出主链结构，但文档里写着——运行本直接读它。
