...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Inspector float: portal the node inspector out of the canvas so it pins
		// to the viewport instead of the (possibly off-screen) panel corner.
		let createPortalFn = null;
		try { const rd = require("react-dom"); if (typeof rd.createPortal === "function") createPortalFn = rd.createPortal; } catch {}
...
const REL_FILE_RE = new RegExp('(?:^|[\s"\'(=|:])([A-Za-z0-9_@%+=:./~-]+\.(?:' + ALL_EXT.join("|") + '))(?![A-Za-z0-9])', "g");
		...
		const CSS = [
			...
			".rb-tail-wrap{display:flex;flex-direction:column;gap:2px;}",
			".rb-tail-edited{padding-top:0;}",
			".rb-tail{display:flex;align-items:center;gap:8px;padding:2px 0 4px;}",
			".rb-tail-label{font-size:11px;color:var(--dsw-alias-label-tertiary);flex:none;}",
			".rb-tail-lane{display:flex;gap:6px;align-items:center;overflow-x:auto;max-width:100%;padding-bottom:2px;}",
			".rb-tail-chip{display:inline-flex;border-radius:8px;overflow:hidden;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);text-decoration:none;line-height:0;}",
			".rb-tail-chip:hover{border-color:var(--dsw-alias-state-business-primary,#2563eb);}",
			".rb-tail-img{width:52px;height:40px;object-fit:cover;display:block;}",
			".rb-tail-chip-doc{line-height:1.4;padding:5px 10px;}",
			".rb-tail-doc{font-size:11.5px;color:var(--dsw-alias-label-secondary);}",
			...
			".rb-inspector{overscroll-behavior:contain;width:300px;max-width:calc(100vw - 32px);max-height:calc(100vh - 32px);overflow-y:auto;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 12px 34px rgba(0,0,0,.28);padding:12px 13px;font-size:12px;animation:rb-insp .18s ease-out;}",
			".rb-insp-float{position:fixed;right:16px;bottom:16px;z-index:2147480000;}",
			".rb-insp-inline{position:absolute;right:12px;bottom:12px;z-index:5;}",
			...
			".rb-narr{overscroll-behavior:contain;border-top:1px solid var(--dsw-alias-border-l1);margin-top:4px;padding-top:8px;max-height:240px;overflow-y:auto;}",
		...
		const dicts = {
			...
			"rb.shelf": "Background · files only mentioned in output",
				"rb.tailLabel": "🖼 run artifacts",
				"rb.tailEdited": "edited this turn",
		...
		var narrBotByTurn = new Map();
		var narrSeenSeq = new Set();
		const userNarrOf = (turnNum) => (narrUserByTurn.get(Number(turnNum)) || "");
		const botNarrOf = (turnNum) => (narrBotByTurn.get(Number(turnNum)) || "");
		const artifactDefinition = {
			...
			match: (event) => {
				try {
					...
					if (type === "user/message" && data !== null && typeof data === "object" && Array.isArray(data.content)) {
						...
						let txt = "";
						// Per-block filter: a checkpoint summary or system reminder riding as
						// the SECOND block of a user message dodges a whole-string prefix
						// check — machine narration must never masquerade as "you said".
						// Harness narration (checkpoint bridges live only in the socket
						// stream, never on disk) must not masquerade as "you said".
						const NOISE = (x) => x.startsWith("<system-reminder>") || x.includes("<compacted-summary>") || x.startsWith("[Request interrupted") || x.startsWith("background job") || x.startsWith("This is an automatically generated checkpoint") || x.startsWith("The user is asking");
						for (const b2 of data.content) {
							if (b2 === null || typeof b2 !== "object" || typeof b2.text !== "string") continue;
							const piece = b2.text.trim();
							if (piece.length === 0 || NOISE(piece)) continue;
							txt += (txt === "" ? "" : "\n") + piece;
						}
						txt = txt.trim();
						if (txt.length > 0) {
			...
			update: (context, match) => {
				...
				try {
					...
					// FULL-text map (seq-deduped): the clipped per-turn storage
					// amputates the middle of long turns — exactly where filenames
					// live. Matching and the "full quote" view need the whole text.
					const bseq = match.event.seq;
					if (bseq === undefined || !narrSeenSeq.has(bseq)) {
						let fullAdd = "";
						if (msg !== null && typeof msg === "object" && Array.isArray(msg.content)) {
							for (const b3 of msg.content) { if (b3 !== null && typeof b3 === "object" && b3.type === "text" && typeof b3.text === "string") fullAdd += (fullAdd === "" ? "" : "\n") + b3.text; }
						}
						if (fullAdd.length > 0) {
							if (bseq !== undefined) narrSeenSeq.add(bseq);
							const tid = match.event.data && match.event.data.turn;
							if (tid !== undefined) {
								const prevF = narrBotByTurn.get(Number(tid)) || "";
								if (prevF.length < 24000) narrBotByTurn.set(Number(tid), (prevF === "" ? "" : prevF + "\n") + fullAdd.slice(0, 24000 - prevF.length));
							}
						}
					}
		...
		// Turn data is pure over the turn's CURRENT artifact values — memoize on
		// the turn object, but revalidate the artifact object identity: a LIVE
		// turn's value is replaced as events stream in, and the cache must follow.
		// The inspector loop and every graph rebuild call this per turn per render;
		// live-flush re-renders made it a repeated O(events) tax for zero change.
		const turnDataCache = new WeakMap();
		function collectTurnData(turn) {
			const art = turn.data.get("runbook-artifacts");
			const hit = turnDataCache.get(turn);
			if (hit !== undefined && hit.art === art) return hit.out;
			const out = collectTurnDataUncached(turn);
			turnDataCache.set(turn, { art, out });
			return out;
		}
		function collectTurnDataUncached(turn) {
			const files = [];
			const seen = new Set();
			let reads = [];
			let activity = [];
		...
		function buildFileGraph(timeline, git, subruns, ledger, scan) {
			...
			// Plausible-FILE gate: the echo harvester's char class includes '=' and ':'
		// so usage text glues flags/variables onto real paths ("--data-dir=PATH",
		// "f_path: /Users/...") — the glued token is not a file and must not enter
		// the graph. A basename with '=', a leading '-', or no dot-dir boundary is
		// text, not a path.
		const isJunkBasename = (b) => b.length === 0 || b.startsWith("-") || /[=:]|^\.+/.test(b) || !/\.[A-Za-z0-9]+$/.test(b);
		const ensure = (path, tIdx) => {
			if (isJunkBasename(basename(path))) return undefined;
			...
			const addEdge = (from, to, kind, tIdx) => {
				...
				seenEdge.add(key);
				// Edge budget: git edges are added LAST but are structural (a commit node
				// without its gitfile edges is unlinked and gets shelved invisible).
				// 150 first-come slots starved them on busy sessions.
				if (edges.length < (kind === "gitfile" || kind === "gitline" ? 320 : 280)) edges.push({ from, to, kind, tIdx, key });
			};
			...
			for (const { i, data } of passRuns) {
				for (const run of data.runs) {
					...
					if (script === undefined) continue; // junk-gated token, not a file
					const outSet = new Set(run.outputs);
					for (const inp of run.inputs) { if (outSet.has(inp)) continue; const n = ensure(inp, i); if (n === undefined) continue; addEdge(n.id, script.id, "consume", i); }
					for (const out of run.outputs) {
						const n = ensure(out, i);
						if (n === undefined) continue; // junk-gated token, not a file
			...
			if (scan !== null && scan !== undefined && Array.isArray(scan.files)) {
				// Session flow is OBSERVED EXECUTION: a scan script enters only if the
				// session itself ran/echoed it (it is already in nodeMap). Eagerly
				// seeding every script on disk made the graph grow with the REPO, not
				// the session — no natural ceiling as a project stabilizes. Un-run
				// architecture lives in the backbone (PIPELINE.md/scan-inferred), not
				// here. Static IO then links only for scripts the session touched,
				// resolving their indirect paths (savefig(str(OUT/…))) against disk.
			...
			// Blind scan fill goes LAST, and only when the session itself shows
			// almost nothing (first-open of an untouched workspace): disk inventory
			// is NOT session flow. Padding a busy graph with every file on disk made
			// the flow grow with the repo instead of the session — no natural
			// ceiling. Session-touched files + linked IO are the bounded truth.
			if (scan !== null && scan !== undefined && Array.isArray(scan.files) && nodeMap.size < 24) {
				for (const f of scan.files) {
					if (nodeMap.size >= 24) break;
			...
			if (Array.isArray(subruns)) {
				...
				for (const sub of subruns) {
					...
					for (const ed of Array.isArray(sub.edits) ? sub.edits.slice(0, 30) : []) {
						...
						// (never reassign the loop const — that crashed every panel on
						// sessions with child-agent edits)
						const edPath = normPath(ed.path);
						if (!isCapturedPath(edPath)) continue;
						const n = ensure(edPath, lastIdx);
						if (n === undefined) continue; // junk-gated token, not a file
			...
			if (git !== null && git !== undefined && git.ok === true && typeof git.cwd === "string" && git.cwd.length > 0) {
				// Multi-repo merge: paths were absolutized per repo in useGitLedger,
				// so root is only a prefix hint now.
				const root = "";
				const times = [];
				...
				if (Array.isArray(git.commits)) {
					...
					for (let ci = git.commits.length - 1; ci >= 0; ci--) {
						...
						for (const f of c.files.slice(0, 10)) {
							const absF = typeof f === "string" ? f : f.p;
							const abs = absF.startsWith("/") ? absF : root + "/" + absF;
							if (nodeMap.has(abs)) { present.push(abs); continue; }
							// Junction aliasing: the session edits the LINKED copy
							// (node_modules/dsh-plugin-runbook/...) while git tracks the
							// real repo path — same file, two paths. A bare basename is
							// ambiguous on machines full of client.js, so the hit must
							// also live inside a "/<repo-name>/" path segment.
							const b2 = basename(absF);
							const repoSeg = "/" + (typeof c.root === "string" && c.root.length > 0 ? basename(c.root) : "") + "/";
							let hit = null;
							for (const cand of nodeMap.values()) {
								if (cand.kind === "commit" || cand.kind === "agent" || basename(cand.path) !== b2) continue;
								// repoSeg confines hits to THIS repo's files, so several
								// hits are just aliases of one file (real repo + junction
								// copy) — take the first instead of bailing.
								if (repoSeg !== "//" && cand.path.indexOf(repoSeg) < 0) continue;
								if (hit === null) hit = cand.path;
							}
							if (hit !== null) present.push(hit);
						}
						// Commits have their OWN budget: file-node exhaustion (big sessions fill 130
						// long before the git pass) must not erase version history from the graph.
					if (present.length === 0 || commitCount >= 16) continue;
			...
			// Structural nodes (commits, agents) enter the map LAST, so a plain
			// slice(0,132) amputated version history on busy sessions. Reserve their
			// seats first, then fill the remainder with file nodes in map order.
			const structural = [...nodeMap.values()].filter((n) => n.kind === "commit" || n.kind === "agent").slice(0, 18);
			const files = [...nodeMap.values()].filter((n) => n.kind !== "commit" && n.kind !== "agent").slice(0, Math.max(24, 132 - structural.length));
			const nodes = structural.concat(files);
		...
		function GraphView(props) {
			...
			// Gesture camera: wheel events write the transform STRAIGHT to the SVG
			// (zero React re-renders mid-gesture, immune to session-flush repaints
			// snapping it back). State only catches up on idle.
			const svgRef = React.useRef(null);
			const camRef = React.useRef({ x: 0, y: 0, s: 1 });
			const camCommit = React.useRef(0);
			// Sync from state ONLY when no gesture is pending: a mid-gesture session
			// flush re-renders with stale state, and overwriting camRef here would
			// snap the camera back mid-pan.
			if (camCommit.current === 0) camRef.current = { x: offset.x, y: offset.y, s: scale };
			const camWrite = (c) => { const el = svgRef.current; if (el !== null) el.style.transform = "translate(" + c.x + "px," + c.y + "px) scale(" + c.s + ")"; };
			const camSet = (c) => {
				camRef.current = c;
				camWrite(c);
				if (camCommit.current !== 0) clearTimeout(camCommit.current);
				camCommit.current = setTimeout(() => { camCommit.current = 0; setScale(c.s); setOffset({ x: c.x, y: c.y }); }, 160);
			};
			const camCancel = () => { if (camCommit.current !== 0) { clearTimeout(camCommit.current); camCommit.current = 0; } };
			...
			// The inspector floats over the VIEWPORT; hide it while the canvas itself
			// is scrolled out of view so it never dangles over unrelated content.
			const [onScreen, setOnScreen] = React.useState(true);
			React.useEffect(() => {
				const el = stageRef.current;
				if (el === null || typeof IntersectionObserver === "undefined") return;
				const io = new IntersectionObserver((es) => { setOnScreen(es[0].isIntersecting); }, { threshold: 0.02 });
				io.observe(el);
			...
			const fit = React.useCallback(() => {
				...
				const c = { x: (r.width - graph.width * s) / 2, y: (r.height - graph.height * s) / 2, s };
				camCancel();
				glide(() => { camRef.current = c; camWrite(c); setScale(c.s); setOffset({ x: c.x, y: c.y }); });
			}, [graph, glide]);
			...
			// Trackpad-first camera: two-finger scroll PANS (the paper follows the
			// fingers), pinch (browsers deliver ctrlKey+wheel) zooms smoothly at the
			// cursor anchor. Plain-wheel-as-zoom made every scroll a scale jump —
			// hostile without a mouse.
			const onWheel = (e) => {
				// Native listener is registered non-passive (effect below), so this
				// preventDefault REALLY swallows the gesture — the browser page must
				// not scroll together with the canvas.
				e.preventDefault();
				userMoved.current = true;
				if (camMode !== "glide") setCamMode("quick");
				if (e.ctrlKey === true || e.metaKey === true) {
					const factor = Math.pow(1.0032, -e.deltaY);
				...
				// Inverted on purpose: the canvas is a VIEWPORT, not a sheet of paper —
				// content moves against the fingers, matching the OS scroll feel.
				setOffset({ x: offset.x - e.deltaX, y: offset.y - e.deltaY });
			};

			// React's synthetic onWheel is a PASSIVE root listener: preventDefault()
			// inside it is silently ignored, so the page scrolled together with the
			// canvas. A native non-passive listener does the real swallowing; a ref
			// carries the freshest handler closure without re-binding.
			const wheelRef = React.useRef(null);
			wheelRef.current = onWheel;
			React.useEffect(() => {
				const el = stageRef.current;
				if (el === null) return undefined;
				const nat = (ev) => { if (wheelRef.current !== null) wheelRef.current(ev); };
				el.addEventListener("wheel", nat, { passive: false });
				return () => { el.removeEventListener("wheel", nat); };
			}, []);
			...
			return React.createElement(React.Fragment, null,
				React.createElement("div", {
					...
					className: "rb-svg",
					ref: svgRef,
					// camRef, not state: a mid-gesture session flush re-renders with
					// stale state — deriving from camRef keeps the DOM transform exact.
					style: { transform: "translate(" + camRef.current.x + "px," + camRef.current.y + "px) scale(" + camRef.current.s + ")", transition: camTrans },
					role: "img",
				},
					...
					selectedId === null ? null : (() => {
						const card = React.createElement(NodeInspector, {
							node: graph.byId.get(selectedId) || null,
							timeline, openFile, runScript, t, backbone,
							explain,
							onExplain: () => { runExplain(graph.byId.get(selectedId) || null); },
							onClose: () => { setSelectedId(null); },
						});
						// Float ABOVE everything at the viewport corner — the panel corner
						// it used to pin to can sit outside the window when the canvas is
						// tall; fixed positioning escapes the canvas entirely.
						if (createPortalFn !== null && typeof document !== "undefined" && document.body !== null) {
							return createPortalFn(React.createElement("div", { className: "rb-insp-float" }, onScreen ? card : null), document.body);
						}
						return React.createElement("div", { className: "rb-insp-inline" }, card);
					})(),
		...
		function NodeInspector(props) {
			const { node, timeline, openFile, runScript, t, onClose, explain, onExplain, backbone } = props;
			...
			const frags = [];
			try {
				// Stage cards carry no file path of their own — their story is the
				// union of the story of every file the stage declares.
				const stageFiles = node.kind === "stage" && backbone !== null && backbone !== undefined
					? ((backbone.nodes.find((n) => "stage:" + n.id === node.id) || {}).files || [])
					: null;
				const base = basename(node.path);
				const hitName = (name, d, uTxt, bTxt) => {
					const b2 = basename(name);
					if (stageFiles === null || !stageFiles.includes(b2)) return false;
					if ((uTxt + "\n" + bTxt).includes(b2)) return true;
					if (d.files.some((x) => basename(x) === b2) || d.reads.some((x) => basename(x) === b2)) return true;
					return d.runs.some((r) => r !== null && typeof r === "object" && ((Array.isArray(r.inputs) && r.inputs.some((x) => basename(x) === b2)) || (Array.isArray(r.outputs) && r.outputs.some((x) => basename(x) === b2))));
				};
				for (let k = timeline.turnOrder.length - 1; k >= 0 && frags.length < 6; k--) {
					...
					const bTxt = botNarrOf(timeline.turnOrder[k]);
					const mentioned = (uTxt + "\n" + bTxt).includes(base);
					const touched = node.kind === "stage"
						? (stageFiles !== null && stageFiles.some((f2) => hitName(f2, d, uTxt, bTxt)))
						: mentioned
					...
					if (bTxt !== "") frags.push({ turn: timeline.turnOrder[k], who: "bot", text: bTxt });
				}
			} catch {}
			const meta = [];
			if (node.producedBy !== null) {
		...
		function useGitLedger(cwd, fallbackDir, candidates) {
			...
			React.useEffect(() => {
				...
				(async () => {
					const tried = new Set();
					const dirs = [];
					if (typeof cwd === "string" && cwd.length > 0) dirs.push(cwd);
					if (typeof fallbackDir === "string" && fallbackDir.length > 0) dirs.push(fallbackDir);
					for (const c of Array.isArray(candidates) ? candidates : []) if (typeof c === "string" && c.length > 0) dirs.push(c);
					// MERGE every repo found: absolutize each repo's commit paths so a
					// single pseudo-ledger can carry multi-repo truth (the conversation
					// commits in a Projects repo while its cwd is the home dir).
					const repos = [];
					for (const d of dirs) {
						if (tried.has(d)) continue;
						tried.add(d);
						const j = await probe(d);
						if (j === null || j.ok !== true || typeof j.cwd !== "string" || j.cwd.length === 0) continue;
						if (repos.some((r) => r.root === j.cwd.replace(/\/+$/, ""))) continue;
						const root = j.cwd.replace(/\/+$/, "");
						// git status entries are OBJECTS {p, s}: plain string concat turned
						// them into "[object Object]" paths.
						const absOf = (f) => {
							const p = typeof f === "string" ? f : (f !== null && typeof f === "object" && typeof f.p === "string" ? f.p : "");
							return p === "" ? f : (p.startsWith("/") ? p : root + "/" + p);
						};
						const absCommits = (Array.isArray(j.commits) ? j.commits : []).map((c) => ({ ...c, root, files: (Array.isArray(c.files) ? c.files : []).map(absOf) }));
						const absDirty = (Array.isArray(j.dirty) ? j.dirty : []).map(absOf);
						repos.push({ root, commits: absCommits, dirty: absDirty });
					}
					if (repos.length === 0) return;
					const merged = { ok: true, cwd: repos[0].root, multiRoot: repos.map((r) => r.root), commits: repos.flatMap((r) => r.commits), dirty: repos.flatMap((r) => r.dirty) };
					// Newest-commits-first ordering across repos keeps bucketOf sane.
					merged.commits.sort((a, b) => (typeof b.at === "number" ? b.at : 0) - (typeof a.at === "number" ? a.at : 0));
					bestRef.current = merged;
					if (alive) setGitData(merged);
				})();
				return () => { alive = false; };
			}, [cwd, fallbackDir, candidates]);
			return gitData;
		}

		// Fetch child-agent runs (cross-session handoff) once per child set.
		function useSubruns(childSessions, sessionId) {
			...
			React.useEffect(() => {
				...
				// The GUI session list never carries subagent rows; the host route
				// self-discovers spawned children from the PARENT's own log.
				if (ids.length === 0 && typeof sessionId === "string" && sessionId.length > 0) {
					let alive = true;
					fetch("agent-subruns", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: [], sessionId }) })
						.then((res) => res.json())
						.then((j) => {
							if (!alive || j === null || typeof j !== "object" || j.ok !== true || !Array.isArray(j.sessions) || j.sessions.length === 0) return;
							setSubruns(j.sessions);
						})
						.catch(() => {});
					return () => { alive = false; };
				}
				if (ids.length === 0) { setSubruns(null); return undefined; }
				let alive = true;
		...
		function RunbookView(props) {
			const { useSession, openFile, runScript, loadOlder, cwd, childSessions, sessionId, t } = props;
			knownCwd = typeof cwd === "string" ? cwd : "";
			const subruns = useSubruns(childSessions, sessionId);
			...
			// Git candidates: a conversation often works ACROSS repos (session cwd is
			// the home dir while the real commits land in a Projects repo — exactly
			// the plugin-dev shape). Probe every distinct run-script dir, not one.
			const gitCandidates = React.useMemo(() => {
				if (timeline === null) return [];
				...
				for (const n of timeline.turnOrder) {
					...
					for (const r of data.runs) {
						if (typeof r.script !== "string" || !r.script.startsWith("/Users/") || /\//.test(r.script) === false) continue;
						if (/\/(\.|node_modules|__pycache__|\.venv|venv)\//.test(r.script)) continue;
						const at = r.script.lastIndexOf("/");
						if (at <= 0) continue;
						const dir = r.script.slice(0, at);
						byCount.set(dir, (byCount.get(dir) || 0) + 1);
					}
				}
				return [...byCount.entries()].sort((a, b) => b[1] - a[1]).slice(0, 4).map((e) => e[0]);
			}, [timeline]);
			const gitData = useGitLedger(overrideDir !== null ? overrideDir : cwd, overrideDir !== null ? null : fallbackDir, overrideDir !== null ? [] : gitCandidates);
			const effectiveDir = gitData !== null && typeof gitData.cwd === "string" && gitData.cwd.length > 0 ? gitData.cwd : fallbackDir;
			const ledger = useLedger(overrideDir !== null ? "" : fallbackDir, timeline, sessionId);
		...
		function ZoomImage(props) {
			...
			const onWheel = (e) => {
				// Called from a NATIVE non-passive listener (effect below): the
				// preventDefault actually stops the page from scrolling with the pan.
				e.preventDefault();
				if (e.ctrlKey === true || e.metaKey === true) {
					const next = Math.min(8, Math.max(1, zoom * Math.pow(1.0040, -e.deltaY)));
					setZoom(next);
					if (next <= 1) setOffset({ x: 0, y: 0 });
					return;
				}
				if (zoom > 1) setOffset({ x: offset.x - e.deltaX, y: offset.y - e.deltaY });
			};
			const wheelRef = React.useRef(null);
			wheelRef.current = onWheel;
			React.useEffect(() => {
			...
			return React.createElement("div", { className: "avt-stage", ref: boxRef },
		...
		// ---------- turn-tail run artifacts ----------
		// Core deliverables list only files MUTATED by edit tools (their own
		// documented blind spot: files created by terminal runs — the exact shape
		// of research sessions). We own that data: the runbook accumulator pairs
		// every run with its echoed outputs. Render them as a thumbnail lane right
		// under the deliverables row; each chip is a browser-openable link (the
		// fileview route serves correct mime types).
		const TAIL_VIEWABLE = new Set(["png", "jpg", "jpeg", "webp", "gif", "svg"]);
		function RunTailArtifacts(props) {
			const { matched, t, turn, seq, openFile } = props;
			if (!Array.isArray(matched) || matched.length === 0) return null;
			const items = matched.slice(0, 8);
			const label = typeof t === "function" ? t("rb.tailLabel") : "🖼 运行产物";
			// The tail chain is winner-takes-all: by claiming this turn we also
			// inherit core deliverables' duty. Mirror producedForClosing (seq filter
			// + first-seen dedupe) so edit-tool files stay visible under our rule.
			let edited = [];
			try {
				const del = turn !== null && typeof turn === "object" && turn.data !== null && typeof turn.data === "object" ? turn.data.get("deliverables") : undefined;
				if (del !== undefined && Array.isArray(del.produced)) {
					const cut = typeof seq === "number" ? seq : Number.POSITIVE_INFINITY;
					const seen = new Set();
					for (const pr of del.produced) {
						if (pr === null || typeof pr !== "object" || typeof pr.path !== "string" || typeof pr.seq !== "number" || pr.seq > cut || seen.has(pr.path)) continue;
						seen.add(pr.path);
						edited.push(pr.path);
					}
				}
			} catch { edited = []; }
			edited = edited.slice(0, 6);
			return React.createElement("div", { className: "rb-tail-wrap" },
				React.createElement("div", { className: "rb-tail" },
					React.createElement("span", { className: "rb-tail-label" }, label),
					React.createElement("div", { className: "rb-tail-lane" },
						items.map((p2) => {
							const ext = p2.slice(p2.lastIndexOf(".") + 1).toLowerCase();
							const isImg = TAIL_VIEWABLE.has(ext);
							return React.createElement("a", {
								key: p2, className: "rb-tail-chip" + (isImg ? "" : " rb-tail-chip-doc"),
								href: fileUrl(p2), target: "_blank", rel: "noopener", title: p2,
							}, isImg
								? React.createElement("img", { className: "rb-tail-img", src: fileUrl(p2), alt: basename(p2), loading: "lazy" })
								: React.createElement("span", { className: "rb-tail-doc" }, "📄 " + basename(p2)));
						}),
					),
				),
				edited.length === 0 ? null : React.createElement("div", { className: "rb-tail rb-tail-edited" },
					React.createElement("span", { className: "rb-tail-label" }, "📝 " + (typeof t === "function" ? t("rb.tailEdited") : "编辑产物")),
					React.createElement("div", { className: "rb-tail-lane" },
						edited.map((p3) => React.createElement("button", {
							key: p3, type: "button", className: "rb-tail-chip rb-tail-chip-doc", title: p3,
							onClick: () => { if (typeof openFile === "function") openFile(p3); },
						}, React.createElement("span", { className: "rb-tail-doc" }, basename(p3)))),
					),
				),
			);
		}
		...
		ctx.slots.inject("conversation.chat.turnTail", () => ctx.slots.register(
			{
				...
				id: "runbook-runartifacts",
				order: -10,
				locale: NS,
				select: (owner) => {
					try {
						const art = owner !== null && typeof owner === "object" && owner.turn !== null && typeof owner.turn === "object" && owner.turn.data !== null && typeof owner.turn.data === "object" ? owner.turn.data.get("runbook-artifacts") : null;
						if (art === null || typeof art !== "object" || !Array.isArray(art.runs)) return null;
						const out = [];
						const seen = new Set();
						for (const run of art.runs) {
							for (const o of Array.isArray(run.outputs) ? run.outputs : []) {
								if (typeof o !== "string" || seen.has(o)) continue;
								const ext = o.slice(o.lastIndexOf(".") + 1).toLowerCase();
								if (!TAIL_VIEWABLE.has(ext) && ext !== "pdf") continue;
								seen.add(o);
								out.push(o);
							}
