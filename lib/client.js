...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---------- PIPELINE.md backbone view ----------
		// When the workspace has a curated PIPELINE.md, this is THE structure: stage
		// cards chained along the mermaid DAG, real files attached under their stage,
		// missing ones rendered as dashed ghosts (the gap list made visible).
		function StageView(props) {
			const { pipeline, graph, openFile, runScript, t } = props;
			...
			const laid = React.useMemo(() => {
				const stageNodes = new Map(pipeline.nodes.map((n) => [n.id, n]));
				const byBase = new Map();
				for (const n of graph.nodes) {
					const b = n.path.slice(n.path.lastIndexOf("/") + 1);
					if (!byBase.has(b)) byBase.set(b, n);
				}
				// topological rank with cycle guard
				const preds = new Map();
				...
				for (const [r, ids] of [...cols.entries()].sort((a, b) => a[0] - b[0])) {
					let y = 18;
					for (const id of ids) {
						const st = stageNodes.get(id);
						const files = (st.files || []).slice(0, 8).map((fname) => {
							const real = byBase.get(fname);
							if (real !== undefined) return { ...real, ghost: false };
							return { id: "ghost:" + id + ":" + fname, path: fname, kind: fileKindOf(fname), label: fname, ghost: true, scan: false };
						});
						const box = { id, x: 18 + r * COL_W, y, w: 288, h: 54 + Math.max(1, files.length) * 0 + (files.length > 0 ? 30 + files.length * 34 : 0), rank: r, stage: st };
						stagePos.set(id, box);
						placedStages.push(box);
						files.forEach((f, fi) => {
							fileNodes.push({ ...f, x: box.x + 14, y: box.y + 58 + fi * 34, w: 240, h: 28, stageId: id });
						});
						y += box.h + 26;
					}
				}
				const width = 18 + (cols.size + 1) * COL_W;
				const height = Math.max(120, ...placedStages.map((b) => b.y + b.h)) + 30;
				const edges = pipeline.edges.map((e, i) => ({ ...e, key: "pe" + i, fromPos: stagePos.get(e.from), toPos: stagePos.get(e.to) })).filter((e) => e.fromPos !== undefined && e.toPos !== undefined);
				return { stages: placedStages, files: fileNodes, edges, width, height };
			}, [pipeline, graph]);

			React.useEffect(() => {
				const el = stageRef.current;
				if (el === null || laid.width === 0) return;
				const sc = Math.min((el.clientWidth - 24) / laid.width, (el.clientHeight - 24) / laid.height, 1);
				setView({ x: Math.max(8, (el.clientWidth - laid.width * sc) / 2), y: Math.max(8, (el.clientHeight - laid.height * sc) / 2), scale: sc });
			}, [laid]);

			const onWheel = (e) => {
				e.preventDefault();
				setView((v) => {
					const ns = Math.min(2.5, Math.max(0.25, v.scale * (e.deltaY < 0 ? 1.12 : 0.89)));
					const rect = stageRef.current.getBoundingClientRect();
					const mx = e.clientX - rect.left, my = e.clientY - rect.top;
					return { scale: ns, x: mx - ((mx - v.x) / v.scale) * ns, y: my - ((my - v.y) / v.scale) * ns };
				});
			};

			const statusColor = (st) => st.status === "💀" ? "var(--dsw-alias-state-error-primary,#e5484d)" : st.status === "⚠️" ? "var(--dsw-alias-state-warn-primary,#e6a23c)" : st.status === "🔶" ? "#d19a0a" : "var(--dsw-alias-state-success-primary,#52c41a)";

			const edgePath = (e) => {
				const a = e.fromPos, b = e.toPos;
				const x1 = a.x + a.w, y1 = a.y + 27, x2 = b.x, y2 = b.y + 27;
				const mx = (x1 + x2) / 2;
				return "M " + x1 + " " + y1 + " C " + mx + " " + y1 + ", " + mx + " " + y2 + ", " + x2 + " " + y2;
			};

			return React.createElement("div", { ref: stageRef, className: "rb-stage", style: { cursor: dragRef.current !== null ? "grabbing" : "grab" },
				onWheel,
				onPointerDown: (e) => { dragRef.current = { x: e.clientX, y: e.clientY, vx: view.x, vy: view.y }; e.currentTarget.setPointerCapture(e.pointerId); },
				onPointerMove: (e) => {
				...
				React.createElement("div", { className: "rb-pipe-badge" },
					React.createElement("svg", { width: 10, height: 10 }, React.createElement(StageBadge, { status: pipeline.inferred === true ? "" : "✅", x: 5, y: 5 })),
					(pipeline.inferred === true ? t("rb.pipeInferred") : t("rb.pipeBadge")) + " · " + pipeline.nodes.length + " " + t("rb.stages"),
				),
				React.createElement("div", { style: { position: "absolute", left: view.x, top: view.y, transform: "scale(" + view.scale + ")", transformOrigin: "0 0" } },
					React.createElement("svg", { width: laid.width, height: laid.height, style: { display: "block", overflow: "visible" } },
						React.createElement("defs", null,
							React.createElement("marker", { id: "rb-arrow-flow", markerWidth: 10, markerHeight: 10, refX: 8, refY: 5, orient: "auto" }, React.createElement("path", { d: "M 0 0 L 10 5 L 0 10 z", fill: "var(--dsw-alias-state-business-primary,#2563eb)" })),
						),
						laid.edges.map((e) => React.createElement("g", { key: e.key },
							React.createElement("path", { d: edgePath(e), className: "rb-edge-flow", markerEnd: "url(#rb-arrow-flow)" }),
							e.label !== undefined && e.label !== "" ? React.createElement("text", { x: (e.fromPos.x + e.fromPos.w + e.toPos.x) / 2, y: (e.fromPos.y + e.toPos.y) / 2 + 22, textAnchor: "middle", className: "rb-edge-flowlabel" }, e.label) : null,
						)),
						laid.stages.map((b) => React.createElement("g", { key: b.id },
							React.createElement("rect", { x: b.x, y: b.y, width: b.w, height: 54, rx: 10, className: "rb-node-rect-stage" }),
							React.createElement("text", { x: b.x + 12, y: b.y + 18, className: "rb-node-stage-label" }, (b.stage.label || "").slice(0, 34)),
							React.createElement("text", { x: b.x + 12, y: b.y + 38, className: "rb-node-glyph rb-node-glyph-script", style: { fontSize: "10px" } }, (b.stage.label || "").slice(34, 78)),
							React.createElement(StageBadge, { status: b.stage.status, x: b.x + b.w - 14, y: b.y + 16 }),
							React.createElement("title", null, b.stage.status === "" ? "" : ({ "💀": "missing", "⚠️": "warning", "🔶": "partial", "✅": "complete" }[b.stage.status] || "")),
						)),
						laid.files.map((f) => React.createElement("g", { key: f.id,
							onMouseEnter: () => { setHoverId(f.id); }, onMouseLeave: () => { setHoverId((h) => (h === f.id ? null : h)); },
							style: { cursor: !f.ghost && isRunnable(f.path) ? "pointer" : "default" },
							onClick: () => { if (!f.ghost && isRunnable(f.path) && runScript) runScript(f.path); },
						},
							React.createElement("rect", { x: f.x, y: f.y, width: f.w, height: f.h, rx: 6, className: f.ghost ? "rb-node-rect rb-node-rect-ghost" : "rb-node-rect rb-node-rect-" + f.kind }),
							React.createElement("text", { x: f.x + 8, y: f.y + f.h / 2 + 3.5, className: f.ghost ? "rb-node-label rb-node-label-ghost" : "rb-node-label" }, f.label.length > 26 ? f.label.slice(0, 25) + "…" : f.label),
							React.createElement("title", null, f.ghost ? t("rb.pipeGhost") : f.path),
							!f.ghost && isRunnable(f.path) && hoverId === f.id ? React.createElement("text", { x: f.x + f.w - 14, y: f.y + f.h / 2 + 4, className: "rb-node-run" }, "▶") : null,
						)),
					),
				),
			);
		...
		function GraphView(props) {
			const { timeline, gitData, subruns, ledger, scan, pipeline, openFile, runScript, t } = props;
			...
			const graph = React.useMemo(() => {
				...
				const laid = layoutFileGraph(g.nodes, g.edges, shelfOpen);
			...
			}, [timeline, gitData, subruns, ledger, scan, shelfOpen]);
			...
			for (const edge of graph.edges) {
				...
				edgeEls.push(React.createElement("path", {
					...
					markerEnd: (edge.kind === "cochange" || edge.kind === "gitline") ? undefined : "url(#rb-arrow" + (edge.kind === "consume" ? "-consume" : edge.kind === "ran" || edge.kind === "edit" ? "-agent" : "") + ")",
				}));
			...
			for (const node of graph.nodes) {
				...
				nodeEls.push(React.createElement("g", {
					...
					React.createElement("rect", { x: node.x, y: node.y, width: node.w, height: node.h, rx: 8, className: "rb-node-rect rb-node-rect-" + node.kind }),
					React.createElement("text", { x: node.x + 10, y: node.y + node.h / 2 + 3, className: "rb-node-glyph rb-node-glyph-" + node.kind }, glyphOf(node)),
					React.createElement("text", { x: node.x + 38, y: node.y + node.h / 2 + 4, className: "rb-node-label", title: node.path }, label),
					isRunnable(node.path) ? React.createElement("text", { x: node.x + node.w - 16, y: node.y + node.h / 2 + 4, className: "rb-node-run" }, "▶") : null,
				hoverId === node.id && node.kind !== "commit" && node.kind !== "agent" ? React.createElement("g", { key: "acts", className: "rb-node-acts" },
		...
		function NodeInspector(props) {
			...
			const turn = timeline.turns.get(node.turnNum);
			const data = turn === undefined ? { files: [], reads: [], activity: [], runs: [], model: null } : collectTurnData(turn);
			const actionable = node.kind !== "commit" && node.kind !== "agent";
			const meta = [];
			if (node.producedBy !== null) {
		...
		function RunbookView(props) {
			...
			const [overrideDir, setOverrideDir] = React.useState(null);
			...
			// Full graph for the backbone view's file attachment (basename matching).
			const graphForStage = React.useMemo(() => buildFileGraph(timeline, gitData, subruns, ledger, scan), [timeline, gitData, subruns, ledger, scan]);
			...
			const backbone = React.useMemo(() => {
				...
				const inf = graphForStage !== null ? inferPipeline(graphForStage) : null;
				return inf;
			}, [pipeline, graphForStage]);
			...
			return React.createElement("div", { className: "rb-root" },
				...
				(viewMode === "stage" || (viewMode === "auto" && backbone !== null))
					? React.createElement(StageView, { pipeline: backbone, graph: graphForStage, openFile, runScript, t })
					: React.createElement(GraphView, { timeline, gitData, subruns, ledger, scan, openFile, runScript, t }),
			);
		}
