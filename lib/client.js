...
const REL_FILE_RE = new RegExp('(?:^|[\s"\'(=|:])([A-Za-z0-9_@%+=:./~-]+\.(?:' + ALL_EXT.join("|") + '))(?![A-Za-z0-9])', "g");
		...
		const CSS = [
			...
			".rb-inspector{width:300px;max-width:calc(100vw - 32px);max-height:calc(100vh - 32px);overflow-y:auto;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 12px 34px rgba(0,0,0,.28);padding:12px 13px;font-size:12px;animation:rb-insp .18s ease-out;}",
