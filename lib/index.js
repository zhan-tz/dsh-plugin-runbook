...
const TEXT = new Set(['tex', 'txt', 'md', 'markdown', 'py', 'csv', 'json', 'js', 'mjs', 'cjs', 'ts', 'tsx', 'jsx', 'html', 'htm', 'css', 'scss', 'less', 'sh', 'bash', 'zsh', 'r', 'ipynb', 'yml', 'yaml', 'toml', 'ini', 'cfg', 'c', 'cpp', 'h', 'hpp', 'rs', 'go', 'java', 'kt', 'sql', 'xml'])
    ...
    handler: async (req, res) => {
      try {
        ...
        const [log, status] = await Promise.all([
          runScript('git', ['-C', cwd, 'log', '-n', '60', '--name-status', '--pretty=format:%H%x1f%at%x1f%s'], '', GIT_TIMEOUT_MS),
          runScript('git', ['-C', cwd, 'status', '--porcelain'], '', GIT_TIMEOUT_MS),
        ])
        if (res.headersSent) return
        ...
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' })
        res.end(JSON.stringify({ ok: true, commits, dirty }))
      } catch (error) {
  ...
  const disposeSubruns = webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const ids = payload !== null && typeof payload === 'object' && Array.isArray(payload.ids) ? payload.ids.filter((x) => typeof x === 'string').slice(0, 12) : []
        if (ids.length === 0) { failJson(res, 400, 'agent-subruns: ids required'); return }
    ...
    handler: async (req, res) => {
      try {
        ...
        for (const f of found) {
          ...
          if (!SCRIPT_EXT.has(ext) || scripts.length >= 60 || f.size > 262144) continue
          let code = ''
          ...
          const addRef = (arr, m) => {
            const name = m[1]
            if (name === undefined || !refBasenameOk(name)) return
            const abs = name.startsWith('/') ? name : base + '/' + name
            if (!exists.has(abs) || arr.includes(abs)) return
          ...
          const addLiteral = (arr, m) => {
            ...
            if (name === undefined || !refBasenameOk(name)) return
            const abs = name.startsWith('/') ? name : base + '/' + name
            if (!exists.has(abs) || known.has(abs) || arr.includes(abs)) return
            known.add(abs)
            if (arr.length < 12) arr.push(abs)
          }
          ...
          for (const m of code.matchAll(LITERAL)) {
            const name = m[1]
            if (name === undefined || !refBasenameOk(name)) continue
            const abs = name.startsWith('/') ? name : base + '/' + name
            if (!exists.has(abs) || known.has(abs)) continue
            known.add(abs)
